...
to setup
  clear-all
  create-doves Population1
    [ set color white - 2 + random 3  ;; random shades look nice
      set size 1.5  ;; easier to see
      setxy random-xcor random-ycor
      set flockmates no-turtles ]
   create-sparrows Population2
    [ set color pink - 2 + random 3  ;; random shades look nice
      set size 1.5  ;; easier to see
      setxy random-xcor random-ycor
      set flockmates no-turtles ]
  reset-ticks
...
;;; BREED
to make-baby ;; turtle procedure
  if count flockmates < max-mates [ hatch 1 [ fd 1 lt random 360] ]
end

;;; OVERPOPULATE
to survive ;; turtle procedure
  if count flockmates >= max-mates [ die ]
end

to boom ;; turtle procedure
  let angle heading
  ask other turtles in-radius vision [ turn-away angle max-sense-turn ]
end
